...
export default function MyFiles() {
  ...
  const openEditDialog = (file: any) => {
    setEditingFile(file);
    setEditChildren(file.children.map((c: any) => String(c.child.id)));
    setEditOpen(true);
  };
    ...
    <div className="relative z-10">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => openEditDialog(file)}>
                      <Edit2 />
                    </button>
                    <button
        ...
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t("editChildren")}</DialogTitle>
            </DialogHeader>
            ...
            {children.map((child: any) => (
              <label key={child.id} className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      setEditChildren([...editChildren, String(child.id)]);
                    } else {
                      setEditChildren(
                        editChildren.filter((id) => id !== String(child.id)),
                      );
                    }
                  }}
                />
